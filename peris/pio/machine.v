...
module machine (
    ...
    input  wire [31:0] input_pins,
    input  wire [ 7:0] irq_flags_in,
    input  wire        imm,
    input  wire        empty,
    input  wire        full,
    input  wire        restart,
    ...
    input  wire [5:0]  pins_out_count,
    input  wire [4:0]  pins_set_base,
    input  wire [2:0]  pins_set_count,
    input  wire [4:0]  pins_in_base,
    input  wire [4:0]  pins_side_base,
    input  wire [2:0]  pins_side_count,
    input  wire        out_shift_dir,
    input  wire        in_shift_dir,
    input  wire        auto_pull,
    input  wire        auto_push,
    input  wire [5:0]  isr_threshold,
    input  wire [5:0]  osr_threshold,
    ...
    reg [5:0]   set_shift_out_count;
    reg [31:0]  already_shift_val;
    ...
    wire pin0_dir= pin_directions[0];
    wire pin1_dir= pin_directions[1];
    ...
    always @ (posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            end else if (!waiting && !exec && delay > 0) begin
                delay_cnt <= delay;
            end
        end
    end
    ...
    always @ (*) begin
        ...
        set_shift_in = 0;
        set_shift_out = 0;
        set_shift_out_count = 0;
        already_shift = 0;
        already_shift_val = 0;
        do_in_shift = 0;
        do_in_shift_continuously = 0;
        do_out_shift = 0;
        decx = 0;
        decy = 0;
        setx = 0;
            ...
            case (op)
                JMP: begin
                        ...
                        case (condition) // Condition
                            0: jmp = 1;                                            // always
                            1: jmp = (x == 0);                                     // !X
                            2: begin jmp = (x != 0); decx = (x != 0); end          // X--
                            3: jmp = (y == 0);                                     // !Y
                ...
                WAIT: case (source2) // Source
                        0: waiting = input_pins[index] != polarity;                // GPIO
                ...
                IN: if (auto_push && isr_count >= isr_threshold) begin // Auto push
                        ...
                        1: do_shift_in(x);                              // X
                        2: do_shift_in(y);                              // Y
                        3: do_shift_in(null);                           // NULL(all zeroes)
                        6: do_shift_in(in_shift);                       // ISR
                        7: do_shift_in(out_shift);                      // OSR
                    endcase
    ...
    isr shift_in (
        ...
        .do_shift_con(do_in_shift_continuously),
        .din        (new_val),
        .dout       (in_shift),
        .bit_count  (bit_count),
    ...
    osr shift_out (
        ...
        .shift      (op2),
        .set        (set_shift_out),
        .set_count  (set_shift_out_count),
        .do_shift   (do_out_shift),
        .din        (already_shift ? already_shift_val : new_val),
        .dout       (out_shift),
